...
6. **记忆播放进度**：播放时程序自动记录观看位置（每 3 秒保存一次到本地 `progress.json`，不影响播放流畅度）。下次打开同一视频自动从上次位置继续，底部会出现橙色提示「已恢复到上次播放位置 时:分:秒」；视频库新增「上次看到」列。**在 PotPlayer 里拖动进度条也会被记录**——播放请求经由程序内置的本地代理转发（仅本机 127.0.0.1，视频数据实时透传、不落盘），拖动产生的跳变会被识别并换算成时间位置；顺序播放到片尾则判定"看完"，下次自动从头播放。右键任意视频可选**从头播放**（清除该片的进度记录）或**清除本片进度记录**。可在设置页关闭此功能。
...
| **续播位置有偏差** | 拖动位置按片内码率换算（PotPlayer 是独立进程，程序拿不到它的内部进度条），前几分钟偏差可能稍大，看完一段时间后自动校准变准；需精确重置时右键该视频选「从头播放」或「清除进度记录」 |
| **日志提示「本地播放代理启动失败」** | 代理端口被安全软件拦截时自动退回直链播放，播放不受影响，只是拖动位置无法记录；将 QuarkPlay.exe 加入白名单即可恢复 |
...
- 程序只与本机 OpenList（127.0.0.1:5244）和 PotPlayer 交互；播放流量经本机代理（仅监听 127.0.0.1）实时透传给 OpenList，再由 OpenList 直连夸克服务器，代理不缓存、不落盘。
